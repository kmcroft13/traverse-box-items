...
## CSV ##
Instead of iterating through all users, the script can be configured to pull a pre-defined set of items to process from a CSV file. The CSV requires 3 columns: `item_id`, `owner_login`, and a `type` column OR a `path` column. This is useful if you already know the scope of the items you want to take action on (for example, but pulling a Shared Link or Folders and Files report from Box).
